...
$orders_stmt = $pdo->prepare("SELECT * FROM sell_records WHERE sell_date = ? ORDER BY id DESC");
...
.date-wrap label{font-size:12px;font-weight:600;color:var(--text-m);}
.date-input{height:36px;padding:0 12px;border:1px solid var(--border);border-radius:8px;font-size:13px;font-weight:600;color:var(--text);background:var(--surface-s);backdrop-filter:blur(8px);outline:none;font-family:inherit;cursor:pointer;transition:border-color .17s,box-shadow .17s;}
...
.order-card:hover{transform:translateY(-2px);box-shadow:0 8px 24px var(--brand-g);}
.order-card.cancelled{border-left:4px solid var(--danger);opacity:.88;}
...
.order-card:nth-child(6){animation-delay:.24s}
...
.order-hdr-left{display:flex;align-items:center;gap:10px;flex-wrap:wrap;}
.inv-badge{
  display:inline-flex;align-items:center;gap:5px;
  font-size:13px;font-weight:700;color:var(--brand);
}
.inv-badge svg{width:14px;height:14px;stroke:var(--brand);fill:none;stroke-width:2;stroke-linecap:round;flex-shrink:0;}
.order-date{font-size:11.5px;color:var(--text-m);display:flex;align-items:center;gap:4px;}
.order-date svg{width:12px;height:12px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;}

.order-hdr-right{display:flex;align-items:center;gap:10px;}
.status-pill{
  padding:3px 10px;border-radius:20px;
  font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;
}
.status-pill.ok{background:var(--brand-g);color:var(--brand);}
.status-pill.can{background:var(--danger-bg);color:var(--danger);}
.order-total{
  font-size:15px;font-weight:800;color:var(--brand);
}
.order-total.cancelled-amt{color:var(--danger);text-decoration:line-through;}

/* Items table inside card */
.items-wrap{overflow-x:auto;}
.items-tbl{width:100%;border-collapse:collapse;}
.items-tbl th{font-size:10.5px;font-weight:700;color:var(--text-m);text-transform:uppercase;letter-spacing:.4px;padding:8px 18px;text-align:left;border-bottom:1px solid var(--border);}
.items-tbl td{padding:10px 18px;font-size:13px;color:var(--text);border-bottom:1px solid var(--border);}
.items-tbl tbody tr:last-child td{border-bottom:none;}
.items-tbl tbody tr{transition:background .13s;}
.items-tbl tbody tr:hover{background:var(--brand-g);}
...
.sub-cell{text-align:right;font-weight:700;}
...
.empty-state svg{width:44px;height:44px;stroke:var(--border-s);fill:none;stroke-width:1.4;stroke-linecap:round;margin-bottom:12px;}
.empty-state p{font-size:13.5px;color:var(--text-m);font-weight:500;}
...
@media(max-width:600px){
  ...
  .items-tbl th,.items-tbl td{padding:8px 12px;}
}
</style>
</head>
<body>
...
<aside id="sb">
  ...
  <nav class="sb-nav">
    ...
    <div class="nsub on" id="ps">
      ...
      <a href="view_pos_entries.php" class="ni act"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>View Entries Log</a>
...
<main id="main">
  ...
  <div class="pg-hdr">
    <div class="pg-hdr-left">
      <h2>Detailed POS Entry Logs</h2>
      <p>Invoice-by-invoice breakdown for selected date</p>
    </div>
  ...
  <?php if(count($orders) > 0):
    foreach($orders as $order):
      ...
      // Fetch items for this specific order — original logic intact
      ...
      <div class="order-hdr">
        <div class="order-hdr-left">
          ...
          <div class="order-date">
            ...
            <?php echo date('d M Y', strtotime($order['sell_date'])); ?>
          </div>
        </div>
        <div class="order-hdr-right">
          <span class="status-pill <?php echo $is_cancelled ? 'can' : 'ok'; ?>">
            <?php echo $is_cancelled ? 'Cancelled' : 'Completed'; ?>
          </span>
          <span class="order-total <?php echo $is_cancelled ? 'cancelled-amt' : ''; ?>">
          ...
          <tbody>
            <?php foreach($order_items as $item): ?>
              <tr>
                ...
                <td class="sub-cell">Rs. <?php echo number_format($item['price'] * $item['quantity'], 0); ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
  ...
  <?php endforeach;
    ...
    <div class="empty-state">
      ...
      <p>No POS checkouts found for <?php echo date('d M Y', strtotime($selected_date)); ?></p>
    </div>
  <?php endif; ?>
