...
<title>Category Setup – Memon Biryani</title>
...
/* ══════════════════════════════ MAIN ══════════════════════════════ */
#main{margin-top:60px;padding:24px 18px;max-width:860px;margin-left:auto;margin-right:auto;position:relative;z-index:1;}
...
/* CARD */
.card{background:var(--surface);backdrop-filter:blur(var(--blur));-webkit-backdrop-filter:blur(var(--blur));border:1px solid var(--border);border-radius:var(--radius);padding:22px;box-shadow:0 4px 20px rgba(0,0,0,.07);transition:background .35s;animation:fadeSlideUp .45s ease both;margin-bottom:18px;}
.card-title{font-size:14px;font-weight:700;color:var(--text);margin-bottom:18px;padding-bottom:10px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;}
...
/* ALERT */
.alert{padding:10px 13px;border-radius:8px;font-size:12.5px;font-weight:600;margin-bottom:16px;display:flex;align-items:center;gap:8px;animation:alertIn .3s ease both;}
...
[data-theme="dark"] .add-row input{background:rgba(255,255,255,.06);}
.add-row input:focus{border-color:var(--brand);box-shadow:0 0 0 3px var(--brand-g);}
.add-row input::placeholder{color:var(--text-m);}
.btn-add{
...
.btn-add svg{width:14px;height:14px;stroke:#fff;fill:none;stroke-width:2.5;stroke-linecap:round;}
...
tbody tr:nth-child(1){animation-delay:.04s}tbody tr:nth-child(2){animation-delay:.07s}tbody tr:nth-child(3){animation-delay:.10s}tbody tr:nth-child(4){animation-delay:.13s}tbody tr:nth-child(5){animation-delay:.16s}tbody tr:nth-child(6){animation-delay:.19s}tbody tr:nth-child(7){animation-delay:.22s}tbody tr:nth-child(8){animation-delay:.25s}
tbody tr:hover{background:var(--brand-g);}
tbody tr:last-child td{border-bottom:none;}

.badge-id{display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:7px;background:var(--brand-g);color:var(--brand);font-size:11px;font-weight:700;}

/* INLINE EDIT */
.cat-display{display:flex;align-items:center;gap:6px;}
.cat-name{font-weight:500;}
.inline-edit-form{display:none;align-items:center;gap:6px;flex-wrap:wrap;}
...
[data-theme="dark"] .inline-edit-form input{background:rgba(255,255,255,.07);}
.inline-edit-form input:focus{border-color:var(--brand);box-shadow:0 0 0 3px var(--brand-g);}
...
.save-btn{color:var(--success);border-color:currentColor;}
.save-btn:hover{background:var(--success-bg);}
.cancel-btn{color:var(--text-m);}
.cancel-btn:hover{background:var(--brand-g);}

.empty-row td{text-align:center;padding:28px;color:var(--text-m);font-size:13px;}

/* STATS BADGE */
.count-badge{display:inline-flex;align-items:center;justify-content:center;padding:2px 9px;border-radius:20px;background:var(--brand-g);color:var(--brand);font-size:11px;font-weight:700;margin-left:6px;}
...
<aside id="sb">
  ...
  <nav class="sb-nav">
    ...
    <div class="nsub on" id="es">
      ...
      <a href="expense_categories.php" class="ni act"><svg viewBox="0 0 24 24"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>Category Setup</a>
    </div>
  ...
  <div class="nm-list">
    ...
    <?php else: foreach($notifications as $n): ?>
      <div class="nm-it">
        ...
        <form method="POST" action="dashboard.php" style="margin:0"><input type="hidden" name="clear_notif_id" value="<?php echo $n['id']; ?>"><button type="submit" class="nm-clr">Clear</button></form>
      </div>
...
<main id="main">
  <div class="pg-hdr">
    <h2>Expense Category Setup</h2>
    <p>Create, rename or remove expense categories</p>
  </div>
  ...
  <div class="card">
    ...
    <form method="POST">
      <div class="add-row">
        <input type="text" name="category_name" placeholder="e.g. Kitchen Expense, Utility Bills…" required>
        ...
        <tbody>
          <?php if(count($categories)>0): foreach($categories as $cat): ?>
            <tr id="row-<?php echo $cat['id']; ?>">
              <td><span class="badge-id"><?php echo $cat['id']; ?></span></td>
              <td>
                <!-- Display mode -->
                <div class="cat-display" id="disp-<?php echo $cat['id']; ?>">
                  <span class="cat-name"><?php echo htmlspecialchars($cat['category_name']); ?></span>
                </div>
                <!-- Edit mode -->
                <form method="POST" id="eform-<?php echo $cat['id']; ?>" style="display:none;">
                  <div class="inline-edit-form" style="display:flex;">
                    <input type="hidden" name="category_id" value="<?php echo $cat['id']; ?>">
                    <input type="text" name="category_name" value="<?php echo htmlspecialchars($cat['category_name']); ?>" required>
                    <button type="submit" name="edit_category" class="action-btn save-btn">
                      <svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>Save
                    </button>
                    <button type="button" class="action-btn cancel-btn" onclick="cancelEdit(<?php echo $cat['id']; ?>)">
              ...
              <td style="text-align:center;">
                <div id="actions-<?php echo $cat['id']; ?>" style="display:flex;gap:6px;justify-content:center;">
          ...
          <?php endforeach; else: ?>
            <tr class="empty-row"><td colspan="3">No categories found. Create one above!</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
...
function showEdit(id){
  ...
  document.getElementById('eform-'+id).style.display='block';
}
function cancelEdit(id){
  document.getElementById('disp-'+id).style.display='flex';
  document.getElementById('actions-'+id).style.display='flex';
  document.getElementById('eform-'+id).style.display='none';
}
